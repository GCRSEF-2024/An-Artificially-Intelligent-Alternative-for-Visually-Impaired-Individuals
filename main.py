...
def MCSMove(SleepTime, throttle=1):
	ThrottleAllMotors(throttle)
...
#Time for 90* = 
def TurnL(TurnTime, TurnThrottle=1):
	ThrottleAllMotors(0)
	MotorR.throttle = TurnThrottle
	time.sleep(TurnTime)
	ThrottleAllMotors(0)

def TurnR(TurnTime, TurnThrottle=1):
	ThrottleAllMotors(0)
	MotorL.throttle = TurnThrottle
	time.sleep(TurnTime)
	ThrottleAllMotors(0)

# Read Image
cap = cv2.VideoCapture('tttff.mp4')
count = 0
...
while cap.isOpened():
	...
	for c in cntrs:
		...
		if 30 < area < 400000:  # Adjust the area range based on your specific case
			...
			if 10 < aspect_ratio < 50.0 and 30 < width < 100 and abs(rect[2]) < 200:
				cv2.drawContours(contours, [c], -1, (0, 0, 255), 1)
				good_contours.append(c)
	...
	if abs(intersection_x) < 3800:
		print("Moving forward - crossin da crosswalk")
		time.sleep(2)
		TurnR(2,1)
		#time.sleep(1)
		MCSMove(3, 1)
		...
		print("Moving right (really going straight)")
		MCSMove(0.5, 1)
		#time.sleep(1)
	ThrottleAllMotors(0)
	#time.sleep(0.5)
